...
const MOCK_REQUESTS: ServiceRequest[] = [
  { id: 1,  title: 'HVAC Unit Installation',      description: 'Full installation of industrial HVAC unit. Requires crane access.', customer_id: 1, customer: CUSTOMERS[0], zone_id: 1, zone: ZONES[0], work_type_id: 1, work_type: WORK_TYPES[0], priority: 2, status: 'scheduled',   created_at: d(2024, 4, 1),  updated_at: d(2024, 4, 2, 14) },
  ...
  { id: 5,  title: 'Network Infrastructure',       description: 'Replace aging network switches and re-run Cat6 cabling throughout building.', customer_id: 1, customer: CUSTOMERS[0], zone_id: 1, zone: ZONES[0], work_type_id: 1, work_type: WORK_TYPES[0], priority: 2, status: 'scheduled',   created_at: d(2024, 4, 3),  updated_at: d(2024, 4, 4) },
...
let nextId = MOCK_REQUESTS.length + 1;
let nextAuditId = MOCK_AUDIT.length + 1;
...
export class DataService {
  ...
  private sim<T>(value: T, ms = 350): Observable<T> {
    return of(value).pipe(delay(ms + Math.random() * 150));
  }
  ...
  getRequests(
    ...
    page: number,
    pageSize: number,
  ): Observable<PagedResult<ServiceRequest>> {
    return this._requests.pipe(
      ...
      map(all => {
        ...
        const items = list.slice(page * pageSize, (page + 1) * pageSize);
  ...
  createRequest(data: Partial<ServiceRequest>): Observable<ServiceRequest> {
    const newReq: ServiceRequest = {
      ...
      customer: CUSTOMERS.find(c => c.id === data.customer_id),
      zone_id: data.zone_id ?? null,
      zone: ZONES.find(z => z.id === data.zone_id),
      work_type_id: data.work_type_id ?? 1,
      work_type: WORK_TYPES.find(wt => wt.id === data.work_type_id),
      priority: (data.priority ?? 3) as Priority,
      status: (data.status ?? 'new') as RequestStatus,
      created_at: new Date().toISOString(),
      updated_at: new Date().toISOString(),
    };
    const updated = [newReq, ...this._requests.value];
    this._requests.next(updated);
    this._addAudit('request', newReq.id, 'create', 'Request created');
    return this.sim(newReq, 600);
  }
  ...
  updateRequest(id: number, data: Partial<ServiceRequest>): Observable<ServiceRequest> {
    ...
    const detail = oldStatus !== updated.status
      ? `status: ${oldStatus} → ${updated.status}`
      : 'Request updated';
    this._addAudit('request', id, 'update', detail);
    return this.sim(updated, 600);
  }

  deleteRequest(id: number): Observable<void> {
    this._requests.next(this._requests.value.filter(r => r.id !== id));
    this._addAudit('request', id, 'delete', 'Request deleted');
    return this.sim(undefined as void, 500);
  }
    ...
    return this._requests.pipe(
      ...
      map(all => {
        const byStatus: Record<string, number> = {
          new: 0, scheduled: 0, en_route: 0, in_progress: 0, done: 0, cancelled: 0,
        };
        ...
        const active = (byStatus['new'] ?? 0) + (byStatus['scheduled'] ?? 0) +
          (byStatus['en_route'] ?? 0) + (byStatus['in_progress'] ?? 0);
        const recent = [...all]
          .sort((a, b) => b.updated_at.localeCompare(a.updated_at))
          .slice(0, 5);
        return {
          total: all.length, active, done: byStatus['done'] ?? 0,
          cancelled: byStatus['cancelled'] ?? 0, byStatus, byPriority, recentRequests: recent,
        };
      }),
  ...
  private _addAudit(entity: string, entityId: number, action: string, details: string): void {
    const entry: AuditLog = {
      id: nextAuditId++,
      entity, entity_id: entityId, action, actor: 'admin',
      details, created_at: new Date().toISOString(),
    };
    this._audit.next([entry, ...this._audit.value]);
  }
}
